...
int Statement::maxargs() {
  ...
  table.push_back([&](struct Q &q) {
    auto e = q.ptr.expr;
    switch (e->kind) {
    case OpExp:ADD_Q(position, QExpr, expr, e->u.op.left);
      ADD_Q(position, QExpr, expr, e->u.op.right);
      break;
    case EseqExp:ADD_Q(position, QStm, stm, e->u.eseq.stm);
      ADD_Q(position, QExpr, expr, e->u.eseq.expr);
      break;
    case IdExp:
    case NumExp:break;
    default:throw UNSUPPORT_TYPE;
      break;
    }
  });
  table.push_back([&](struct Q &q) {
    auto l = q.ptr.list;
    switch (l->kind) {
    case PairExpList:ADD_Q(position, QExpr, expr, l->u.pair.head);
      ADD_Q(position, QList, list, l->u.pair.tail);
      break;
    case LastExpList:ADD_Q(position, QExpr, expr, l->u.last);
      break;
  ...
  table.push_back([&](struct Q &q) {
    auto s = q.ptr.stm;
    switch (s->kind) {
    case CompoundStm:ADD_Q(position, QStm, stm, s->u.compound.stm1);
      ADD_Q(position, QStm, stm, s->u.compound.stm2);
      break;
    case AssignStm:ADD_Q(position, QExpr, expr, s->u.assign.expr);
      break;
    case PrintStm: {
...
struct Result Expression::eval(Table *table) {
  ...
  switch (kind) {
  case NumExp:r1.caculated = u.num;
    r1.table = table;
    return r1;
  case IdExp:r1.caculated = table->lookup(u.id);
    r1.table = table;
    return r1;
  case OpExp:r1 = u.op.left->eval(table);
